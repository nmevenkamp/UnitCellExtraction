...
RealType periodicityEnergy1D ( const PictureType &Data, const RealType &Angle, const RealType Period ) {
  ...
  for ( int y=0; y<Data.getNumY ( ) ; ++y ) {
    for ( int x=0; x<Data.getNumX ( ) ; ++x ) {
      ...
      if ( ( shiftedPos[0] >= 0 && shiftedPos[0] < Data.getNumX ( ) && shiftedPos[1] >= 0 && shiftedPos[1] < Data.getNumY ( ) )
          && ( !aol::isNaN<RealType> ( Data.get ( x, y ) ) && !aol::isNaN<RealType> ( Data.interpolate ( shiftedPos ) ) ) ) {
  ...
  void getFundamentalPeriodsFromPeriodicityEnergyMinimization ( aol::Vector<RealType> &FundamentalPeriods, aol::Vector<RealType> &LatticeAngles, const PictureType &Data ) {
    ...
    for ( int i=0; i<LatticeAngles.size ( ) ; ++i ) {
      ...
      if ( localMinima.size ( ) > 0 ) {
        // Step 2: Determine local minimum that corresponds to smallest spacing and which belongs to the same class of local minima as the global minimum
        const RealType delta = 0.2;
        if ( localMinima.size ( ) == 1 ) {
          FundamentalPeriods[i] = localMinimaSpacings[0];
        } else {
          // Step 2.1: Throw away all local minima whose energy is not below the threshold (absolute minimum + delta)
          const RealType threshold = localMinima.getMinValue ( ) + delta;
          int j=0;
          ...
          if ( localMinima.size ( ) == 1 ) {
            ...
            // Step 2.2 Linear correction of energy penalty due to errors in angle estimation
            ...
            // Step 2.3: Cluster local Minima using k-Means and determining k based on Akaike's Information Criterion
            ...
            // Step 2.4: Select cluster with smallest mean (i.e. the one corresponding to the global minimum)
            ...
            // Step 2.5: From cluster with smallest mean, select local minimum that corresponds to smallest period among all that are within the threshold of the absolute minimum
