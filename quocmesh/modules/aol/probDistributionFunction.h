...
public:
  ...
  static RealType PDF ( const RealType X, const RealType Mean = 0, const RealType Variance = 1 ) {
    ...
    return invSqrtPi * inv_sigma_sqrt_2 * exp ( -aol::Sqr<RealType> ( ( X - Mean ) * inv_sigma_sqrt_2 ) );
  }
...
template <typename RealType> RealType NormalDistribution<RealType>::invSqrtPi = 1.0 / sqrt ( aol::NumberTrait<double>::pi );
