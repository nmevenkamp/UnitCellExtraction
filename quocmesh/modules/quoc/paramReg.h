 ...
 * \author Berkels
 */
template <typename _ConfiguratorType, bool AllowScaling = false>
...
public:
  ...
  void setDeformationParametersAsVec ( const aol::Vector<RealType> &DeformParametersVec ) {
    ...
    _rotationAngle = DeformParametersVec[ConfiguratorType::Dim];
    _scaling = ( AllowScaling ? DeformParametersVec[ConfiguratorType::Dim+1] : 1 );
    _rotationMatrix.makeRotation ( _rotationAngle );
  }
  ...
  static void setIdentityDeformationParametersAsVec ( aol::Vector<RealType> &DeformParametersAsVec ) {
    ...
    if ( AllowScaling )
      DeformParametersAsVec[ConfiguratorType::Dim + 1] = 1;
  }
  ...
  static void getDeformationParametersOfInverse ( const aol::MultiVector<RealType> &DeformPars, aol::MultiVector<RealType> &InverseDeformPars )  {
    InverseDeformPars[1][0] = -DeformPars[1][0];
    aol::Matrix22<RealType> inverseRotationMatrix;
    inverseRotationMatrix.makeRotation ( InverseDeformPars[1][0] );
    ...
    if ( AllowScaling ) {
      InverseDeformPars[ConfiguratorType::Dim][0] = aol::ZOTrait<RealType>::one / DeformPars[ConfiguratorType::Dim][0];
      inverseTranslation *= InverseDeformPars[ConfiguratorType::Dim][0];
    }
  ...
  void evaluateDerivativeDeformationOn0N ( const typename ConfiguratorType::VecType &Position,
                                          aol::Mat< NumberOfDeformParameters, ConfiguratorType::Dim, RealType > &Jacobian ) const {
    evaluateDerivativeDeformationOn01 ( Position, Jacobian );
  ...
  void evaluateSecondDerivativeDeformationOn0N ( const typename ConfiguratorType::VecType &Position,
    ...
    for ( int i = 0; i < Dim; i++ ) {
      Hessian[i].setZero();
      Hessian[Dim][i].setZero();
    }

    Hessian[Dim][Dim][0] = ( -cosAlpha * ( Position[0] - 0.5 ) + sinAlpha * ( Position[1] - 0.5 ) );
    Hessian[Dim][Dim][1] = ( -sinAlpha * ( Position[0] - 0.5 ) - cosAlpha * ( Position[1] - 0.5 ) );

    if ( AllowScaling ) {
      for ( int i = 0; i < Dim; i++ )
        Hessian[Dim+1][i].setZero();
      Hessian[Dim][Dim+1][0] = Hessian[Dim+1][Dim][0] = ( -sinAlpha * ( Position[0] - 0.5 ) - cosAlpha * ( Position[1] - 0.5 ) );
      Hessian[Dim][Dim+1][1] = Hessian[Dim+1][Dim][1] = (  cosAlpha * ( Position[0] - 0.5 ) - sinAlpha * ( Position[1] - 0.5 ) );
      Hessian[Dim+1][Dim+1].setZero();
    }
  }
  ...
  void evaluateMixedHessianOn0N ( const typename ConfiguratorType::VecType &/*Position*/,
    ...
    Hessian[Dim][0][0] = -sinAlpha;
    Hessian[Dim][0][1] = -cosAlpha;
    Hessian[Dim][1][0] =  cosAlpha;
    Hessian[Dim][1][1] = -sinAlpha;

    if ( AllowScaling ) {
      Hessian[Dim] *= _scaling;
      Hessian[Dim+1] = _rotationMatrix;
    }
  }
